...
typedef struct timer_root_ {
    CIRCLEQ_HEAD(timer_bucket_root_, timer_bucket_)
    timer_bucket_qhead; /* Bucket list  */
    CIRCLEQ_HEAD(timer_gc_root_, timer_)
    timer_gc_qhead; /* Garbage collection list */
    CIRCLEQ_HEAD(timer_change_root_, timer_)
    timer_change_qhead; /* Change timers list */
 ...
 * Since time does not run backwards, timer insertion becomes a O(1) operation
 * as one needs only to locate the appropriate bucket and insert at the tail of
 * the per bucket queue.
 */
...
typedef struct timer_ {
    ...
    uint expired : 1, periodic : 1, /* auto restart timer ? */
 ...
 */
void
timer_set_expire(timer_s *, time_t, long);

/*
 * Public API.
 */
void
timer_init_root(timer_root_s *);
void
timer_flush_root(timer_root_s *);
void
timer_test(void *);
void
timer_add(timer_root_s *, timer_s **, char *, time_t, long, void *, void *);
void
timer_add_periodic(timer_root_s *, timer_s **, char *, time_t, long, void *, void *);
void
timer_del(timer_s *);
void
timer_smear_bucket(timer_root_s *, time_t, long);
void
timer_walk(struct timer_root_ *);
...
char *
timespec_format(struct timespec *);
